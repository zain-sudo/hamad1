...
#pragma warning restore CA1506
        ...
        private void ValidateConfigurationSettings()
        {
            ...
            if (string.IsNullOrWhiteSpace(this.configuration.GetValue<string>("EwsServer")) || string.IsNullOrWhiteSpace(this.configuration.GetValue<string>("EwsServer")))
            {
                throw new ApplicationException("EWS URL is missing in the configuration file.");
            }
        }
        ...
        private void GetKeyVaultByManagedServiceIdentity()
        {
            ...
            using (var keyVaultClient = new KeyVaultClient(new KeyVaultClient.AuthenticationCallback(azureServiceTokenProvider.KeyVaultTokenCallback)))
            {
                ...
                var ewsUrlValue = keyVaultClient.GetSecretAsync($"{this.configuration["KeyVault:BaseURL"]}{this.configuration["KeyVaultStrings:EwsSecretUrl"]}");

                ewsUrlValue.Wait();
                this.configuration["EwsServer"] = ewsUrlValue.Result.Value;
            }
        }
    }
}