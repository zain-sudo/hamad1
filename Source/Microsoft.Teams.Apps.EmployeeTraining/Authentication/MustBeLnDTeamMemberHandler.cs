...
namespace Microsoft.Teams.Apps.EmployeeTraining.Authentication
{
    ...
    public class MustBeLnDTeamMemberHandler : AuthorizationHandler<MustBeLnDTeamMemberRequirement>
    {
        ...
        {
            this.telemetryClient.TrackTrace("ValidateUserIsPartOfTeamAsync CALLED");

            // The key is generated by combining teamId and user object id.
            this.telemetryClient.TrackEvent("isCacheEntryExists CHECK");
            ...
            {
                this.telemetryClient.TrackEvent("Cache entry does not exists");

                try
                {
                    this.telemetryClient.TrackEvent("UserValidMember CHECK");
                    ...
                    {
                        this.telemetryClient.TrackEvent("Member is valid");
                        this.telemetryClient.TrackTrace("ValidateUserIsPartOfTeamAsync SUCCESS");
                        return isUserValidMember;
                    }
                    else
                    {
                        this.telemetryClient.TrackException(new Exception("Invalid member"));
                        this.telemetryClient.TrackException(new Exception("ValidateUserIsPartOfTeamAsync FAIL"));
                        return isUserValidMember;
                    }
                }
                catch (Exception ex)
                {
                    this.telemetryClient.TrackException(new Exception($"ValidateUserIsPartOfTeamAsync FAIL {ex.Message}"));
                }
            }
