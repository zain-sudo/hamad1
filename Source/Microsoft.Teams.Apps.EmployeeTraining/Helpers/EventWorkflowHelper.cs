...
namespace Microsoft.Teams.Apps.EmployeeTraining.Helpers
{
    ...
    public class EventWorkflowHelper : IEventWorkflowHelper
    {
        ...
        /// <returns>Boolean indicating save operation result.</returns>
        public async Task<bool?> CreateNewEventAsync(EventEntity eventEntity, string createdByName, TelemetryClient telemetryClient)
        {
            eventEntity = eventEntity ?? throw new ArgumentNullException(nameof(eventEntity), "Event details cannot be null");

            telemetryClient.TrackEvent($"CreateNewEventAsync started.");
            ...
            if (!string.IsNullOrEmpty(eventEntity.EventId))
            {
                ...
                telemetryClient.TrackEvent($"CreateNewEventAsync with event Id  null started.");
            ...
            else
            {
                ...
                telemetryClient.TrackEvent($"CreateNewEventAsync with event Id  else.");

                // Create new event record in storage.
                this.MapEventModel(eventEntity, eventToUpsert, isEdit: false, isDraft: false);
            }

            telemetryClient.TrackEvent($"Calculate occurrences.");
            ...
            telemetryClient.TrackEvent($"Set time of selected start date.");
            ...
            telemetryClient.TrackEvent($"Set time of selected end date.");
            ...
            telemetryClient.TrackEvent($"Set SelectedUserOrGroupListJSON.");
            ...
            {
                telemetryClient.TrackEvent($"ValidateMandatoryAndOptionalUsers started.");
                eventToUpsert = await this.ValidateMandatoryAndOptionalUsers(eventToUpsert);
                telemetryClient.TrackEvent($"ValidateMandatoryAndOptionalUsers end.");
            }

            telemetryClient.TrackEvent($"eventGraphHelper.CreateEventAsync");

            // Create event using MS Graph.
            dynamic graphEventResult = null;
            try
            {
                graphEventResult = await this.eventGraphHelper.CreateEventAsync(eventToUpsert, telemetryClient);
                telemetryClient.TrackEvent($"eventGraphHelper.CreateEventAsync end");
            ...
            catch (Exception ex)
            {
                ...
                telemetryClient.TrackEvent($"{ex.InnerException}");
                telemetryClient.TrackEvent($"{ex.StackTrace}");
            }

            telemetryClient.TrackEvent($"SendEventCreationNotificationInTeam started.");

            var activityId = await this.SendEventCreationNotificationInTeam(eventToUpsert, createdByName);

            telemetryClient.TrackEvent($"SendEventCreationNotificationInTeam completed.");
        ...
        /// <param name="telemetryClient">telemetry</param>
        /// <returns>Boolean indicating save operation result.</returns>
        public async Task<bool?> UpdateEventAsync(EventEntity eventEntity, TelemetryClient telemetryClient)
        {
            eventEntity = eventEntity ?? throw new ArgumentNullException(nameof(eventEntity), "Event details cannot be null");
            if (string.IsNullOrEmpty(eventEntity.EventId))
            {
            ...
            return await this.retryPolicy.ExecuteAsync<bool?>(async () =>
            {
                ...
                telemetryClient.TrackEvent($"event workflow registered attendees {eventEntity.RegisteredAttendees}");
                telemetryClient.TrackEvent($"event workflow registered attendees {eventEntity.AutoRegisteredAttendees}");
                ...
                // Create event using MS Graph.
                var graphEventResult = await this.eventGraphHelper.UpdateEventAsync(eventToUpsert, telemetryClient);
        ...
        public async Task<bool> UpdateEventStatusAsync(string teamId, string eventId, EventStatus eventStatus, string userAadId, TelemetryClient telemetryClient)
        {
            // Retry policy to handle consistency while updating event.
            return await this.retryPolicy.ExecuteAsync<bool>(async () =>
            {
                ...
                {
                    await this.eventGraphHelper.CancelEventAsync(eventDetails.GraphEventId, eventDetails.CreatedBy, this.localizer.GetString("CancelEventComment"), telemetryClient);
